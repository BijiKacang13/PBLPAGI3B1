...
export default function LogAktivitasPage() {
  ...
  const fetchLog = async () => {
    try {
      ...
      // Note: axios interceptor already returns response.data, so res is already the data object
      ...
      if (res?.data?.data && Array.isArray(res.data.data)) {
        // Pagination response: res.data.data (karena axios interceptor sudah return response.data)
        arr = res.data.data;
      } else if (Array.isArray(res?.data)) {
        // Non-pagination response: res.data
        arr = res.data;
      ...
      // Better error logging - err is normalized object from axios interceptor
      // with shape: { status: number, message: string }
      const errorMessage = err?.message || "Terjadi kesalahan saat mengambil data";
      const errorStatus = err?.status || "Unknown";

      console.error("Gagal fetch log aktivitas:");
      ...
      if (err?.status === 401) {
        ...
        setError("Endpoint log aktivitas tidak ditemukan.");
